...
void Motor::bepaalPID(){
	...
	if((LFS[0]== 0 )&&(LFS[1]== 1 )&&(LFS[2]== 0 )&&(LFS[3]== 0 ))  { error = 3;limitReached=false;} //car must turn to the right (All the sensors are between the dotted line and the full line)
	else if((LFS[0]== 0 )&&(LFS[1]== 1 )&&(LFS[2]== 1 )&&(LFS[3]== 0 ))  { error = 3;limitReached=false;} //The car is over the white line: error
	else if((LFS[0]== 0 )&&(LFS[1]== 0 )&&(LFS[2]== 1 )&&(LFS[3]== 1 ))  { error = 2; limitReached=false;} //The car has to turn to the right (the two sensors on the right are on the full line)
	else if((LFS[0]== 0 )&&(LFS[1]== 0 )&&(LFS[2]== 0 )&&(LFS[3]== 1 ))  { error = 1;limitReached=false;} //the sensor on the right is on the full line, the sensor next to it is just next to the full line, this is the situation we want
	else if((LFS[0]== 0 )&&(LFS[1]== 0 )&&(LFS[2]== 0 )&&(LFS[3]== 0 ))  { error = -1;limitReached=false;} //car must turn to the right (All the sensors are between the dotted line and the full line)
